...
class M_Buku extends Model
{
    ...
    public function getDataBukuJoin($where = false)
    {
        if ($where === false) {
            ...
            $builder->select('*');
            $builder->join('siswa_kelas','siswa_kelas.id_siswa = data_siswa.id_kelas', 'LEFT');
            $builder->join('jurusan','jurusan.id_jurusan = data_siswa.id_jurusan', 'LEFT');
            $builder->orderBy('data_siswa.nama_siswa','ASC');
